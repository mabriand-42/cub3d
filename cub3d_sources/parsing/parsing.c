...
int		ft_get_config(char *id, char *line, size_t *pos, t_config *c)
{
	if (ft_strcmp(id, "R") == 0)
		...
		return (ft_text(line, pos, &(c->T.path_NO), &(c->check.NO_texture)));
	else if (ft_strcmp(id, "SO") == 0)
		return (ft_text(line, pos, &(c->T.path_SO), &(c->check.SO_texture)));
	else if (ft_strcmp(id, "WE") == 0)
		return (ft_text(line, pos, &(c->T.path_WE), &(c->check.WE_texture)));
	else if (ft_strcmp(id, "EA") == 0)
		return (ft_text(line, pos, &(c->T.path_EA), &(c->check.EA_texture)));
	else if (ft_strcmp(id, "S") == 0)
	{
		(*pos)--;
		return (ft_text(line, pos, &(c->T.path_S), &(c->check.S_texture)));
	}
