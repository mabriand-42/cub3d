...
int		ft_cmp_char(char c1, char c2, t_map *map)
{
	...
	{
		if (map->cardinal == Not_given)
			ft_update_player(c1, &m); //changed
		else
			return (0);
	}
...
int		ft_duo_line(char *next, char *prev, t_map *map)
{
	...
	{
		if ((ret = ft_cmp_char(next[i], prev[j], map)) == 1)
		{
			if ((ret2 = ft_cmp_prev_next(prev[i], next, j) == 1))
			{
				i++;
				j++;
			}
		...
		else if ((ret = ft_cmp_char(next[i], prev[j], map)) == 0)
			break ;
	}
